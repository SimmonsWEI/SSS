...
from ultralytics.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                    ...
                                    GhostBottleneck, GhostConv, Segment, C2fCA, SPPFCSPC_group, C3STR, C3STR2, SPPFCSPC)
...
class BaseModel(nn.Module):
    ...
    def _forward_once(self, x, profile=False, visualize=True):
        ...
        for m in self.model:
            ...
            visual = Path('runs/features')
            if  visualize:
                # print(m.type, m.i)
                feature_visualization(x, m.type, m.i, save_dir=visual)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus,
                 BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, nn.ConvTranspose2d, DWConvTranspose2d, C3x, C2fCA, C3STR, C3STR2, SPPFCSPC_group, SPPFCSPC):
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(c2 * gw, 8)

            args = [c1, c2, *args[1:]]
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, C2fCA, C3STR, C3STR2):
