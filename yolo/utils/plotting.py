...
def feature_visualization(x, module_type, stage, save_dir=Path('runs/features')):
    """
    x:              Features to be visualized
    module_type:    Module type
    stage:          Module stage within model
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            ...
            merged_feature_map = np.mean(x[0].cpu().detach().numpy(), axis=0)  # merge all channels into one feature map
            plt.imshow(merged_feature_map)  #cmap='gray'
            plt.axis('off')
            LOGGER.info(f'Saving {f}...')
            plt.savefig(f, dpi=600, bbox_inches='tight')
            plt.close()
            np.save(str(f.with_suffix('.npy')), merged_feature_map)  # npy save
